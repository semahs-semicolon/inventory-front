<script>
	...
	$: if (parentId != 'undefined')
		parent = authfetch(`${API_URL()}/categories/${parentId}`).then((a) => a.json());
	else parent = Promise.resolve().then((a) => ({ name: '루트' }));

	const create = async () => {
		madeReq = true;
		try {
			let fileData = undefined;
			if (fileupload != null) fileData = await fileupload;
			...
			const res = await authfetch(`${API_URL()}/categories`, {
				...
				body: JSON.stringify({
					...
					primaryImage: fileData?.id,
					parentCategoryId: parentId == 'undefined' ? null : parentId
				})
			});
