...
<div class="wrapper">
	...
	<span class="name">
		{#if category.categoryId != null}
			{#if category.parentCategoryId != null}
				<PrimaryButton on:click={() => goto(`/dashboard/category/${category.parentCategoryId}`)}
					>&lt;</PrimaryButton
				>
			{:else}
		...
		<slot />
		{#if category.categoryId != null}
			<PrimaryButton on:click={() => goto(`/dashboard/category/${category.categoryId}/edit`)}
				>수정</PrimaryButton
			>
		{/if}
		<PrimaryButton on:click={() => goto(`/dashboard/category/create?parent=${category.categoryId}`)}
			>+</PrimaryButton
		>
	</span>
	<hr />
	<div class="categoryList">
		{#each childCategories
			...
			.sort((a, b) => a.name.localeCompare(b.name)) as child}
	...
	<div class="productList">
		{#each category.products
			...
			<div class="product">
				<Product {product} on:dblclick={goto(`/dashboard/products/${product.id}`)} />
			</div>
		{/each}
	</div>
</div>
