<script>
	...
	import PlainLocation from '../../products/[id]/PlainLocation.svelte';
		...
		<div class="relative">
			...
			<div class="controls">
				<div class="controls-top">
					...
					{/if}
					{#if location > 0}
						<PrimaryButton on:click={() => goto(`/dashboard/tree/${routeIds[location - 1]}`)}
							>메인에서 열기</PrimaryButton
						>
					{:else}
				...
				<span class="route">
					{#each route as segment}
						<span class="segment" class:segment-selected={segment.id == visibleTree.id}
							>{segment.name}</span
						>
