<script>
	...
	let page = 0;
	let size = 100;
	let elements = authfetch(`${lastURL}?size=${size}&page=${page}`)
		.then((a) => a.json())
		.then((a) => (lastSearchResult = a));
	...
	const loadMore = () => {
		...
		elements = authfetch(`${lastURL}?size=${size}&page=${page}`)
			.then((a) => a.json())
			.then((res) => {
				lastSearchResult = [...lastSearchResult, ...res].filter(
		...
		{#if selected != null}
			<div class="imageandproduct">
				...
				<div class="product">
					<PrimaryButton
						on:click={() => {
							goto(`/dashboard/products/${selected.id}`);
						}}>물품으로 이동하기</PrimaryButton
					>
